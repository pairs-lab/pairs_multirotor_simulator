...
#   /src     -> the colcon src/ directory containing all the pairs_* packages
...
LOCAL_RULES="/src/pairs_multirotor_simulator/packaging/rosdep/pairs.yaml"
...
build_one() {
  ...
  if [ ! -d "$pkg_dir" ]; then
    echo "ERROR: dependency '$pkg_name' not found at $pkg_dir — is it checked out in src/ on its ros2 branch?" >&2
    exit 1
  fi
  echo "=================================================================="
